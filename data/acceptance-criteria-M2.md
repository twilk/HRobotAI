...
> **Status:** ✅ mapowanie a–f kompletne z dowodami (kod, testy, zielone runy CI/CD, zrzuty w raporcie KM2). Sekcja 1 ([4M] — dosłowne brzmienie umowy i wzór protokołu) uzupełniana przy podpisaniu protokołu na sesji odbiorczej 17–20.07.
...
| **b** | Agent AI Grafik Manager | serwis Python: `/agent/propose|feedback|heal|explain|forecast`, pętla uczenia (affinity-learner + batch re-fit), wersjonowana polityka | J4 (live demo + Reset&Replay), wykres AG2 spadku edit-distance | ✅ demonstrowalne · ujęcie zgodne z known-limitations |
...
| **f** | UAT Etapu 2 | scenariusze J1–J5 + skrypt UAT + zamrożone dane syntetyczne + wewnętrzny cykl testów (QA 3 ról, 2 próby generalne) | sesja z 4Mobility + protokół — element odbioru 17–20.07 | ✅ przygotowane do odbioru |

## 3. Kryteria techniczne (z specyfikacji) — stan po testach

| Kryterium | Stan | Uwaga / dowód |
|---|:--:|---|
| G1 pokrycie reguł | ✅ w przyjętym zakresie | **H1–H4 twarde** (zweryfikowane testami); H5 jako reguła miękka; H6/fairness — KM3 (spec przeredagowana, commit D1). |
| G2 metryki (dojazdy/etaty) | ✅ | commute+etatDeviation realne; człon fairness — KM3 |
| G3 determinizm | ✅ | deterministyczny tryb single-worker + stały seed (przyjęty zakres gwarancji) |
| G4 INFEASIBLE jawnie | ✅ | zwraca `unmet[]` |
| G5 web-kit na realnym API | ✅ | zweryfikowane wizualnie (:5601, 52 AUTO-zmiany) |
| G6 izolacja tenantów | ✅ | potwierdzona testami jednostkowymi izolacji; test integracyjny na żywych bazach zaplanowany (CI-4, KM3) |
...
| CI-1..6, ENV-1..3 | ✅ | CI-1/CI-3/CI-6 wdrożone (branch protection, py-lanes); ENV-1 zielony run 29374277217 (14/14); CI-4/CI-5 zaplanowane wraz z realnymi testami (KM3) |
