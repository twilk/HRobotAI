...
| **STT Agenta Głosowego** *(zaktualizowane 2026-08-13)* | Warstwa mowa→tekst jest **udokumentowanym szwem poza modułem NestJS** i tak pozostaje: `STT_PORT` w tenant-runtime **nadal nie ma adaptera**, a moduł kadrowy **nigdy nie styka się z audio** — dostaje wyłącznie tekst, tak samo od klawiatury jak od mikrofonu. To granica projektowa, nie zaległość. Od M3 działa natomiast **osobna usługa** faster-whisper `small` (PL), wołana z warstwy web: zbudowana, uruchomiona i będąca częścią ścieżki demo. Audio przetwarzane w pamięci, **nie persystowane**, poza infrastrukturę nie wychodzi. | `stt.port.ts` (interfejs bez implementacji — celowo); usługa: `stt-service/` + `docker-compose.yml` (`stt`, port kontenera 8010); konsument: `apps/web/app/api/voice/transcribe/route.ts`. |
...
| **Dane** *(zaktualizowane 2026-08-13)* | Wszystkie trzy moduły działają na danych **syntetycznych**. Kotwice demo urosły do **39 pracowników i 1558 zmian** — zbiór rozszerzono, by grupy porównawcze modułu rozwoju miały sens statystyczny (próg 5 osób). Zero realnego PII bez zmian. | SPEC Dokumenty §8; [l-modul-analityk-hr.md](l-modul-analityk-hr.md) §2 (greenfield, zero realnego PII). |
