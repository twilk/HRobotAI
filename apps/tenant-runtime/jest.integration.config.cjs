// Integration lane [CI-4]: the tests that need a REAL Postgres (>=2 tenant databases).
//
...
//
// Runs serially with a long timeout: these specs CREATE DATABASE / migrate / DROP DATABASE against
// a shared Postgres, so parallel workers would race on the same cluster.
