...
import { IsNotSameTimeAs, IsRealCalendarDate } from './grafik-validators.js'
...
export class CreateShiftDemandDto {
  ...
  @IsRealCalendarDate({ message: 'date must be an ISO YYYY-MM-DD calendar date' }) date!: string
  @Matches(HHMM, { message: 'start must be HH:mm' }) start!: string
  @Matches(HHMM, { message: 'end must be HH:mm' })
  @IsNotSameTimeAs('start')
  end!: string
...
export class UpdateShiftDemandDto {
  ...
  @IsOptional() @IsRealCalendarDate({ message: 'date must be an ISO YYYY-MM-DD calendar date' }) date?: string
