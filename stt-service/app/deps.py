"""Keycloak bearer auth for the ``/voice/*`` surface.

COPIED, deliberately, from ``agent-service/app/deps.py`` — the PoC report
(``docs/superpowers/specs/2026-07-21-agent-glosowy-poc.md`` §1) says the auth pattern is to be
reused verbatim rather than re-invented per service: "Wzorzec auth (``deps.py``) kopiujemy/
wydzielamy, nie duplikujemy logiki tenantowej na własną modłę." The two services are separate
images with no shared Python package, so a byte-for-byte copy with this note is the honest way to
keep one behaviour in two places. If a shared package ever appears, both should import it instead.
That copying is also why this file inherited the track-J issuer bug and is fixed in lockstep.
...
that is the M2 tenant-isolation fix (AG6).
...
(``RS256``); ``verify_aud`` is disabled because the STT service is not itself an audience of the
