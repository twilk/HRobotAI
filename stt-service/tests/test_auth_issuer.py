...
The stt-service copy of these tests exists because its own suite satisfies ``require_tenant``
with ``dependency_overrides`` and therefore never exercises the real issuer path at all.

These tests drive ``require_tenant`` directly rather than through a TestClient, because
``tests/test_api.py`` stubs the dependency out entirely for the app fixtures — and it is precisely
the real ``_jwks`` path that must be exercised here.
"""
